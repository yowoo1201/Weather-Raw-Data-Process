...
`Input/` 폴더는 빈 상태로 깃에 올라가 있습니다. 다음 파일들을 직접 받아 넣어야 합니다:
The `Input/` folder is empty in git. Fetch and place these manually:

| 파일 / File | 출처 / Source |
|---|---|
| `Input/oisst_data/oisst-avhrr-v02r01.YYYYMMDD.nc` | NOAA OISST v2.1 (<https://www.ncei.noaa.gov/data/sea-surface-temperature-optimum-interpolation/v2.1/access/avhrr/>) |
| `Input/nino_clim_daily_1991-2020.csv` | 사전 산출 / pre-computed (별도 setup) |
| `Input/wksst9120.for.txt` | CPC `wksst9120.for` (<https://www.cpc.ncep.noaa.gov/data/indices/wksst9120.for>) |
| `Input/rel_wksst9120.txt` | CPC relative SSTA |
| `Input/data.tar` | PMEL TAO/TRITON `t<lat><n\|s><lon><e\|w>_dy.ascii.gz` 묶음 (<https://www.pmel.noaa.gov/tao/data_deliv/deliv-nojava-all.html>) |
| `Input/data_p_clim_processed.tar.gz` | per-lon DOY climatology builder 산출 |
